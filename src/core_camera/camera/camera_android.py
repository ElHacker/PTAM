...
class CameraAndroid(CameraBase):
    ...
    def init_camera(self):
        ...
        '''
        with self._fbo:
            self._texture_cb = Callback(self._my_texture_cb)
            Rectangle(size=self._resolution)

    def _my_texture_cb(self, instr):
        Logger.info("Texture Callback called")
        return self._camera_texture.bind
    ...
    def _on_preview_frame(self, data, camera):
        with self._buflock:
            ...
            self._buffer = data
            Logger.info("On Preview FRAME")
            # CUSTOMIZED
            buf = self._buffer.tostring()
            processed_data = self._process_frame(buf)
            self._buffer = processed_data

    # Do the custom processing of a frame
    def _process_frame(self, buf):
        ...
        # arr[:, 0:640] = 0
        result_buffer = arr.flatten()
        result_buffer = arr.tobytes()
        Logger.info("Buffer equals")
        Logger.info(buf == result_buffer)
    ...
    def decode_frame(self, buf):
        ...
        # arr = cvtColor(arr, cv2.CV_BGR2GRAY)
        return arr
